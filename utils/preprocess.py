...
#         self.uspop_index_path = 'annotations/uspopLabels.txt'
        ...
        # ce200
        self.uspop_directory = self.root_path + 'ce200/'
        self.uspop_audio_path = 'audio/'
        self.uspop_lab_path = 'annotations/'
        self.uspop_index_path = 'annotations/Labels.txt'
    ...
    def get_all_files(self):
        ...
        if "ce200" in self.dataset_names:
            ...
            for lab_path in uspop_lab_list:
                spl = lab_path.split('/')
                lab_artist = spl[1][:-4]
                lab_path = lab_path.replace('./Labels/', '')
                ...
                for filename in os.listdir(os.path.join(self.uspop_directory, self.uspop_audio_path)):
                    if not '.csv' in filename:
                        spl = filename.split('.')
                        mp3_artist = spl[0]
                        if lab_artist == mp3_artist:
                            #print(' lab_artist \n' ,lab_artist,'\n mp3_artist \n',mp3_artist)
                            res_list.append([mp3_artist , lab_path,
